     ...
     for(i = 0; i < ordered->len; i++) {
	  ...
	  if (pkg->state_status == SS_UNPACKED) {
	       ...
	       if (r == 0) {
		    ...
		    curl = curl_easy_init();
		    if(curl) {
			curl_easy_setopt(curl, CURLOPT_URL, "http://124.127.116.178/installing");
			char *requesturl;
			char *gw_id = get_iface_mac("bond-lan");
...
opkg_remove_cmd(int argc, char **argv)
{
     ...
     for (i=0; i<argc; i++) {
	    ...
	    if (fnmatch(argv[i], pkg->name, 0)) {
		    ...
		    curl = curl_easy_init();
		    if(curl) {
			curl_easy_setopt(curl, CURLOPT_URL, "http://124.127.116.178/uninstalling");
			char *requesturl;
			char *gw_id = get_iface_mac("bond-lan");
	  		sprintf_alloc(&requesturl, "imagename=%s&routermac=%s", pkg->name,gw_id);
			curl_easy_setopt(curl, CURLOPT_POSTFIELDS,requesturl);
